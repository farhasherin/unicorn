...
function Details(data) {
    ...
    clearInterval(clearC);
     clearC = setInterval(() => {
        const dt = Date.now(); // Get the current date and time in milliseconds
        const month = moment(dt).format('MM');
        const year = moment(dt).format('YYYY'); // Get the current year
        const date = moment(dt).format('DD');
        const day = moment(dt).format('dddd');
        const timezoneInMinutes = data.timezone_offset / 60; // Assuming data.timezone_offset is defined elsewhere
        ...
        dayEI.innerHTML = day + ',';
        dateEI.innerHTML = date + ' ' + months[parseInt(month) - 1] + ' ' + year; // Display the current year
    }, 1000);
...
// ============================================================== + + ===============================================================================
// backgorund according to the time opening the website
...
window.onload = function() {
    ...
    if (hour >= 5 && hour < 11) {
        body.style.backgroundImage = "url('images/back-morning.jpg')";
    } else if (hour >= 11 && hour < 15) {
        body.style.backgroundImage = "url('images/back-noon.jpg')";
    } else if (hour >= 15 && hour < 19) {
        body.style.backgroundImage = "url('images/back-evening.jpg')";
    } else {
        body.style.backgroundImage = "url('images/back-night.jpg')";
    }
}